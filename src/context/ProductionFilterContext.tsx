...
export type ProductionDateMode = "daily" | "monthly" | "yearly";

export interface ProductionFilterRequestParams {
  period: ProductionDateMode;
  date_from: string;
  date_to: string;
  divisi: string;
}
...
interface ProductionFilterContextValue {
  ...
  divisi: string;
  requestParams: ProductionFilterRequestParams;
  requestKey: string;
  months: { value: number; label: string }[];
  years: number[];
  setMode: (mode: ProductionDateMode) => void;
  setDivisi: (divisi: string) => void;
  resetFilters: () => void;
  daily: {
    month: number;
    year: number;
...
const ProductionFilterContext = createContext<ProductionFilterContextValue | undefined>(undefined);
...
interface ProductionFilterProviderProps {
  children: React.ReactNode;
  defaultDivisi?: string;
}

export const ProductionFilterProvider = ({ children, defaultDivisi = "ALL" }: ProductionFilterProviderProps) => {
  const defaultStartYear = CURRENT_YEAR - 1;
  const [mode, setMode] = useState<ProductionDateMode>("daily");
  const [divisi, setDivisi] = useState<string>(defaultDivisi);
  ...
  }, [defaultDivisi, defaultStartYear]);

  const requestParams: ProductionFilterRequestParams = useMemo(() => {
    return {
      period: mode,
  ...
  }, [dateDetails.dateRange.from, dateDetails.dateRange.to, mode, divisi]);

  const requestKey = useMemo(() => JSON.stringify(requestParams), [requestParams]);

  const value: ProductionFilterContextValue = {
    mode,
    modeLabel,
...
export const useProductionFilters = () => {
  const context = useContext(ProductionFilterContext);
  if (!context) {
    throw new Error("useProductionFilters must be used within a ProductionFilterProvider");
  }
  return context;
};

export const productionFiltersToQuery = (filters?: ProductionFilterRequestParams) => {
  if (!filters) return {};
  return {
