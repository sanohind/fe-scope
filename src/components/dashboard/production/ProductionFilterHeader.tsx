...
import { useProductionFilters, ProductionDateMode } from "../../../context/ProductionFilterContext";

interface ProductionFilterHeaderProps {
  title?: string;
  filters?: {
    period?: boolean;
    divisi?: boolean;
...
const MODE_OPTIONS: { value: ProductionDateMode; label: string }[] = [
...
const ProductionFilterHeader: React.FC<ProductionFilterHeaderProps> = ({ title = "Global Filters", filters }) => {
  const { mode, setMode, divisi, setDivisi, months, years, rangeSummary, rangeDescription, resetFilters, daily, monthly, yearly } = useProductionFilters();

  const shouldShowPeriod = filters?.period !== false;
  const shouldShowDivisi = filters?.divisi !== false;
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      {shouldShowDivisi && (
        <div className="mt-6">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Division</label>
          <select
            value={divisi}
...
export default ProductionFilterHeader;
