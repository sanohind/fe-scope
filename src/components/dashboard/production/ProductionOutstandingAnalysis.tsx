...
const ProductionOutstandingAnalysis: React.FC = () => {
  ...
  const getStatusBadgeColor = (status: string) => {
    ...
    if (statusLower.includes("progress") || statusLower.includes("process"))
      return "bg-blue-light-100 text-blue-light-700 dark:bg-blue-light-900/20 dark:text-blue-light-400";
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
        <div className="mb-4">
          <h3 className="font-semibold text-gray-800 text-lg dark:text-white/90">
            Production Outstanding Analysis
          </h3>
        </div>
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Production orders with completion progress
        </p>
      </div>
        ...
        <table className="w-full min-w-[1200px]">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-800">
              ...
              <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase dark:text-gray-400">
                Status
              </th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase dark:text-gray-400">
                Division
            ...
            {data.map((item, index) => (
              ...
              >
                ...
                <td className="px-4 py-4 text-sm text-gray-600 dark:text-gray-400">
                  {item.planning_date ? new Date(item.planning_date).toLocaleDateString() : '-'}
                </td>
                ...
                <td className="px-4 py-4 text-sm text-right text-gray-800 dark:text-white/90">
                  {(item.qty_order || 0).toLocaleString()}
                </td>
                <td className="px-4 py-4 text-sm text-right text-gray-800 dark:text-white/90">
                  {(item.qty_delivery || 0).toLocaleString()}
                </td>
                <td className="px-4 py-4 text-sm text-right text-gray-800 dark:text-white/90">
                  {(item.qty_os || 0).toLocaleString()}
                </td>
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    <div
                      className={`flex-1 h-2 rounded-full ${getProgressBgColor(
                    ...
                    <span className="text-xs font-medium text-gray-700 dark:text-gray-300 min-w-[45px]">
                      {(item.completion_percentage || 0).toFixed(1)}%
                    </span>
                  </div>
                </td>
