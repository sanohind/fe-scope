...
interface ProductionTrendOdooProps {
  dateFrom?: string;
  dateTo?: string;
  period?: "daily" | "monthly" | "yearly";
}

const ProductionTrendOdoo: React.FC<ProductionTrendOdooProps> = ({ dateFrom, dateTo, period = "daily" }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const result = await productionApi.getDailyProductionQtyOdoo({
          period,
          date_from: dateFrom,
          date_to: dateTo,
        });
        ...
        console.error("Error fetching Production Trend Odoo:", err);
  ...
  const chartData = useMemo(() => {
    return data.map((item) => {
      ...
      return {
        ...item,
        qty_actual: toNumber(item.qty_actual),
  ...
  const latestValue = chartData.length ? chartData[chartData.length - 1].qty_actual : null;
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-3 shadow-lg dark:border-gray-700 dark:bg-gray-900">
          ...
          <div className="mt-2 space-y-1">
            <div className="flex items-center justify-between gap-4">
              <p className="text-sm text-gray-500 dark:text-gray-300">Qty Actual:</p>
              <p className="text-sm font-semibold text-[#10B981] dark:text-[#10B981]">{dataPoint.qty_actual.toLocaleString()}</p>
            </div>
            <div className="flex items-center justify-between gap-4">
    ...
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Production Achievement (Odoo)</h3>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {latestValue !== null && (
            <div className="rounded-xl bg-gray-50 px-4 py-2 text-sm font-medium text-gray-700 dark:bg-gray-900 dark:text-gray-300">
              Today's Qty Actual: <span className="text-gray-800 dark:text-gray-200">{latestValue.toLocaleString()}</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="min-w-[600px]">
          <ResponsiveContainer width="100%" height={320}>
            <ComposedChart data={chartData} margin={{ top: 10, right: 20, bottom: 0, left: 0 }}>
              ...
              {/* Bar for Qty Actual */}
              <Bar dataKey="qty_actual" name="Qty Actual" fill="#10B981" radius={[4, 4, 0, 0]} barSize={20} />

              {/* Line for Qty Plan */}
              <Line type="monotone" dataKey="qty_plan" name="Qty Plan" stroke="#465FFF" strokeWidth={3} dot={{ strokeWidth: 2, r: 3 }} activeDot={{ r: 5 }} connectNulls />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </div>
...
export default ProductionTrendOdoo;
