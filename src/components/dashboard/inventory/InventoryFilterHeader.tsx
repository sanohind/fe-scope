...
import { InventoryFilterMode, useInventoryFilters } from "../../../context/InventoryFilterContext";

interface InventoryFilterHeaderProps {
  title?: string;
  filters?: {
    period?: boolean;
    customer?: boolean;
...
const MODE_OPTIONS: { value: InventoryFilterMode; label: string }[] = [
  { value: "daily", label: "Daily" },
  { value: "monthly", label: "Monthly" },
  { value: "yearly", label: "Yearly" },
];

const InventoryFilterHeader: React.FC<InventoryFilterHeaderProps> = ({ title = "Global Filters", filters }) => {
  const { mode, setMode, months, years, rangeSummary, rangeDescription, resetFilters, daily, monthly, yearly, customer, groupType } = useInventoryFilters();

  const shouldShowPeriod = filters?.period !== false;
  const shouldShowCustomer = filters?.customer !== false;
  const shouldShowGroupType = filters?.groupType !== false;
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      {(shouldShowCustomer || shouldShowGroupType) && (
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {shouldShowCustomer && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Customer</label>
              <select
                value={customer.value}
                onChange={(event) => customer.setValue(event.target.value)}
                className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white"
              >
              ...
              {customer.error && <p className="mt-2 text-sm text-error-500">Failed to load customers ({customer.error})</p>}
            </div>
          )}

          {shouldShowGroupType && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Group Type</label>
              <select
                value={groupType.value}
                onChange={(event) => groupType.setValue(event.target.value)}
                className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-900 focus:border-brand-500 focus:ring-2 focus:ring-brand-200 dark:border-gray-700 dark:bg-gray-900 dark:text-white"
              >
                {groupType.options.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
                ))}
              </select>
              {groupType.error && <p className="mt-2 text-sm text-error-500">Failed to load group types ({groupType.error})</p>}
            </div>
          )}
        </div>
      )}
