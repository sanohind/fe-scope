...
const StockByCustomer: React.FC = () => {
  ...
  const selectedData = selectedCustomer === "all" 
    ? { customer: "All Customers", total_onhand: data.total_onhand, total_items: data.total_items }
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div className="space-y-4">
        <div className="rounded-lg bg-blue-50 p-6 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">Total Stock</p>
          <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">
            {selectedData.total_onhand.toLocaleString()}
          </p>
        ...
        {selectedCustomer !== "all" && (
          <div className="rounded-lg bg-gray-50 p-6 dark:bg-gray-900">
            ...
            <p className="text-xl font-bold text-gray-800 dark:text-white/90">
              {selectedData.total_items.toLocaleString()}
            </p>
          </div>
        )}
      </div>
