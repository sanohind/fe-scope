...
interface OrderFulfillmentRateProps {
  warehouse: string;
}

type FilterPeriod = "daily" | "monthly";

const OrderFulfillmentRate: React.FC<OrderFulfillmentRateProps> = ({ warehouse }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await warehouseRevApi.getOrderFulfillmentByTransactionType(warehouse, {
          date_from: dateFrom,
          date_to: dateTo,
        });
  ...
  const categories = data.data.map((item) => item.trx_type);
