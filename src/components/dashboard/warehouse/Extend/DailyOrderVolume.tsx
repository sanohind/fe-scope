...
interface DailyOrderVolumeProps {
  warehouse: string;
}

type FilterPeriod = "weekly" | "monthly";

const DailyOrderVolume: React.FC<DailyOrderVolumeProps> = ({ warehouse }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (filterPeriod === "weekly") {
          ...
          // Default to monthly (selected month)
        ...
        const result = await warehouseRevApi.getDailyOrderVolume(warehouse, {
          period: apiPeriod,
          date_from: dateFrom,
          date_to: dateTo,
        });
        ...
        if (result && Array.isArray(result.data)) {
          ...
          console.error("DailyOrderVolume: Expected data array but got:", result);
          setData([]);
        }
        setError(null);
      } catch (err) {
    ...
    fetchData();
  }, [warehouse, filterPeriod, selectedMonth, selectedYear]);
