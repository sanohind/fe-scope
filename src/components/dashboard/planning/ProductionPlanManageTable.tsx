...
const ProductionPlanManageTable: React.FC = () => {
  ...
  const [editingData, setEditingData] = useState<ProductionPlanData | null>(null);
  const [editFormData, setEditFormData] = useState<Partial<ProductionPlanData>>({});
  ...
  const handleEdit = (row: ProductionPlanData) => {
    setEditingData(row);
    setEditFormData({
      partno: row.partno,
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      setIsSaving(true);
      const response = await productionPlanApi.update(editingData.id, editFormData);
  ...
  const confirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await productionPlanApi.delete(deletingId);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await productionPlanApi.deleteMultiple(selectedIds);
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Manage production plan data</p>
          </div>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} className="max-w-[600px] p-5 lg:p-10">
        <div className="relative w-full overflow-y-auto bg-white rounded-3xl dark:bg-gray-900">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">Edit Production Plan</h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">Update the production plan data.</p>
          </div>
          <form
            ...
            <div className="px-2 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <Input type="number" name="qty_plan" value={editFormData.qty_plan || 0} onChange={(e) => setEditFormData({ ...editFormData, qty_plan: parseInt(e.target.value) || 0 })} min="0" required />
                </div>
                <div>
                  <DatePicker
                    id="edit-plan-date"
