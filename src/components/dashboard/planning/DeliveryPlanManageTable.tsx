...
import { deliveryPlanApi, DeliveryPlanData } from "../../../services/deliveryPlanApi";
...
const DeliveryPlanManageTable: React.FC = () => {
  const [rows, setRows] = useState<DeliveryPlanData[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      if (dateRange) {
        params.delivery_date_from = dateRange.from;
        params.delivery_date_to = dateRange.to;
      }

      const result = await deliveryPlanApi.getAll(params);
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/5">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Delivery Plan Data</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">View delivery plan data</p>
        </div>
...
export default DeliveryPlanManageTable;
